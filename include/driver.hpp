...
namespace yy
{
  ...
  public :
    ...
    { 
      return lexer_.get_current_line();
    }

    const int get_current_column() const noexcept 
    { 
      return lexer_.get_current_column(); 
    }

    CurrentScopeNode* get_current_scope() 
    { 
      assert(scopeStorage.back()); 
    ...
    void set_executable_status(const bool status) noexcept 
    { 
      isExecutable_ = status; 
    } 
    ...
    bool is_variable_declared(const std::string id) const 
    { 
      return find_variable(id) != nullptr; 
    }
